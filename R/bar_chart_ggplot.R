...
#' @param bar_type A string indicating the type of the bars. Can be "stack" (default) or "group".
...
BarChart_ggplot <- function(df,
  ...
  bar_type <- ifelse(bar_type == "group", "dodge", bar_type)
  ...
  p <- ggplot(df, aes(x = .data[[x_col]], y = .data[[y_col]], fill = .data[[color_col]])) +
    geom_bar(stat = "identity", position = bar_type, width = 0.5) +
